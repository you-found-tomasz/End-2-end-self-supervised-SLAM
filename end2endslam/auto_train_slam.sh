...
odometry="gradicp"
train_odometry="gt"
bsub -R "rusage[mem=8096, ngpus_excl_p=1]" python3 pointfusion_scsfm.py --odometry ${odometry} --train_odometry ${train_odometry} --dataset tum --dataset_path $path --sequences ${sequence} --seq_length 10 --batch_size ${batch} --debug_path ${debug_path} --model_name ${sequence}_matthias_gt --loss_photo_factor 1 --loss_geom_factor 0.5 --loss_smooth_factor 0.1 --loss_cons_factor 0 --loss_gt_factor 0 --log_freq 50 --max_scale ${scale} --seq_dilation ${dilation} --max_num_batches ${max_batches} --seq_start 396 --seq_end 571 --num_epochs ${epochs} --projection_mode previous --seq_stride ${stride} --learning_rate ${lr} --freeze ${freeze}
