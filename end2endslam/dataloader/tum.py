#TUM dataloader adapted to for sc-sfml
#the images are rescaled bilinearly from 640x480 to 342x256 (1.875 factor)
#and then cropped horizontally on the sides to 320x256
...
class TUM(data.Dataset):
    ...
    THis is adapted from the TUM dataloader from gradslam. 
    This is an adaption for sc-sfml
    the images are rescaled bilinearly from 640x480 to 342x256 (1.875 factor)
    and then cropped horizontally on the sides to 320x256
    ...
    def __init__(
        ...
        width: int = 342, #ceil(640/1.875), #leave default for sc-sfml compatibility
        cropped_width: int = 320, #width after croppisng (crops equal margin at both sides)
    ...
    def _preprocess_depth(self, depth: np.ndarray):
        ...
        #CROP width to cropped_width
        margin = (depth.shape[1] - self.cropped_width)//2
        depth = depth[:,margin:depth.shape[1]-margin]
